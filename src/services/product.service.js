...
const normalizeTaxonomyValue = (value, taxonomyEntry = null) => {
  ...
  if (
    taxonomyEntry &&
    typeof taxonomyEntry.name === "string" &&
    taxonomyEntry.name.trim()
  ) {
...
const validateTaxonomySelection = (type, value, catalog = []) => {
  ...
  if (!normalizedValue) {
    throw new AppError(
      `${type === "brand" ? "La marca" : "La categoría"} es requerida`,
      400,
      "TAXONOMY_REQUIRED",
    );
  }
  ...
  if (!match) {
    ...
    throw new AppError(
      `${label} no existe o no está activa`,
      400,
      "TAXONOMY_INVALID",
    );
  }
...
class ProductService {
  ...
  async create(productData) {
    ...
    if (sanitizedData.brand) {
      ...
      sanitizedData.brand = validateTaxonomySelection(
        "brand",
        sanitizedData.brand,
        existingBrands,
      );
    ...
    if (sanitizedData.category) {
      ...
      sanitizedData.category = validateTaxonomySelection(
        "category",
  ...
  async update(id, updateData) {
    ...
    const producto = await Producto.findByIdAndUpdate(id, sanitizedUpdate, {
      new: true,
      runValidators: true,
    });
  ...
  async filterByCategory(category) {
    ...
    const query = {
      $or: [
        ...
        {
          category: {
            $regex: categoryPattern || categoryLookup,
            $options: "i",
          },
        },
        {
          category: {
            $regex: categoryLookup.replace(/\s+/g, "[ _-]*"),
            $options: "i",
          },
        },
      ],
  ...
  async filterByBrand(brand) {
    ...
    const query = {
      $or: [
        ...
        {
          brand: {
            $regex: brandLookup.replace(/\s+/g, "[ _-]*"),
            $options: "i",
          },
        },
      ],
